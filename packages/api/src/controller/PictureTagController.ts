...
export async function getPictureTag(parent, args, context, info) {
    const id = args.id;
    const pictureTag = await prisma.pictureTag.findUnique({where: {id}});
    if(!pictureTag) {
        throw new ApolloError(`No PictureTag with ID '${id}' found`, "NOT_FOUND")
    }
    return pictureTag;
}


export async function getPictureTags(parent, args, context, info) {
    ...
    const results = await prisma.$transaction([
        prisma.pictureTag.count({where}),
        prisma.pictureTag.findMany({
            where,
            orderBy,
            take: limit,
            skip: offset
...
export async function getPicturesForPictureTag(parent, args, context, info) {
    ...
    const pictureTag = await prisma.pictureTag.findUnique({
        where: {id},
        include: {
...
export async function createPictureTag(parent, args, context, info) {
    authBlock(context);
    const authorizedGroups = ['picture_tag_manager'];
    ...
    const pictureTag = await prisma.pictureTag.create({data});
    return pictureTag;
}


export async function editPictureTag(parent, args, context, info) {
    authBlock(context);
    const authorizedGroups = ['picture_tag_manager'];
    if(!authedUserInGroups(context, authorizedGroups)) {
        throw new ForbiddenError(`User is not authorized to perform this action. Authorized groups are ${authorizedGroups}`)
    ...
    const pictureTag = await prisma.pictureTag.update({where: {id}, data});
    return pictureTag;
}
...
export async function deletePictureTag(parent, args, context, info) {
    ...
    const id = args.id;
    await prisma.pictureTag.delete({where: {id}});
    return true;
}
