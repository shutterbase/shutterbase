...
**Upload metrics** accumulate on the upload row, no event log: `taggingSeconds` is active interaction time — the gap between two consecutive tagging actions by the upload's owner counts as work only when it is shorter than `TAGGING_IDLE_THRESHOLD` (default 2m), so breaks drop out; `timeToReadySeconds` is wall clock from creation (or from being sent back) to `ready`, summed over every cycle; `reviewCycles` counts submissions. `errorCount` / `imagesPerSecond` / `tagsPerImage` are derived at serialization time from live counts (`repository.GetUploadMetrics`) — `errorCount` is the number of images currently carrying the `error` tag, so clearing the tag lowers it again.
