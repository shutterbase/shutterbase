...
**Upload review flow** (opt-in per project via `project.uploadReviewEnabled`): an upload moves `open` → `ready` → `reviewed`. The photographer (owner) may only submit `open → ready`; sending back, accepting and reopening are the projectAdmin's. Once an upload leaves `open`, its **official** tags (everything except `type=custom`) freeze for the photographer and it accepts no further images; custom tags stay editable because they are never exported. The reserved per-project tag named **`error`** (a custom tag, auto-created when a project opts in) marks a tagging error and only a projectAdmin may add or remove it. Rules live in `authorization.CanTransitionUpload` / `CanAssignTag` / `CanAddImagesToUpload`; the SPA mirrors them in `ui/src/util/uploadReview.ts` (pure, unit-tested) so it never offers a 403. The uploads page renders a kanban board over the three states.

**Upload metrics** accumulate on the upload row, no event log: `taggingSeconds` is active interaction time — the gap between two consecutive tagging actions by the upload's owner counts as work only when it is shorter than `TAGGING_IDLE_THRESHOLD` (default 2m), so breaks drop out; `timeToReadySeconds` is wall clock from creation (or from being sent back) to `ready`, summed over every cycle; `reviewCycles` counts submissions; `errorImageIds` remembers every image ever flagged, so the distinct-error count survives clearing the tag and later cycles. `imagesPerSecond` / `tagsPerImage` are derived at serialization time from live counts (`repository.GetUploadMetrics`).

**Accounts:** self-signup (`POST /auth/signup`, gated by `SELF_SIGNUP_ENABLED`, per-IP rate limited) creates an **inactive** plain-role account — a platform admin activates it from the users page before the first login. go-basicauth's own `/auth/register` stays blocked: its user shape has no firstName/lastName. Global admins add/activate/deactivate users; a **projectAdmin** manages the member roster of their own project (`authorization.CanManageProjectAssignment`).
