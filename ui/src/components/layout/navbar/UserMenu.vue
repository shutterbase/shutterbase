<template>
  <div v-if="loggedIn">
    ...
    >
      ...
      <img class="w-8 h-8 rounded-full" :src="`${headshotUrl}?thumb=128x128t`" alt="user photo" />
    </button>
    <!-- Dropdown menu -->
    <div class="hidden z-50 my-4 w-56 text-base list-none bg-white rounded divide-y divide-gray-100 shadow dark:!bg-gray-700 dark:divide-gray-600 rounded-xl" id="dropdown">
      <div class="py-3 px-4">
        ...
        <span class="block text-sm text-gray-900 truncate dark:text-white">{{ pb.authStore.model?.username }}</span>
      </div>
