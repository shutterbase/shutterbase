...
<template>
  <div class="fixed bottom-3 right-3 z-[9999] font-mono text-xs">
    <button @click="open = !open" class="cursor-pointer rounded-full bg-fuchsia-700 text-white w-10 h-10 shadow-lg hover:bg-fuchsia-600" title="Dev quick-actions">
      {{ open ? "×" : "DEV" }}
    </button>

    <div v-if="open" class="mt-2 w-72 rounded-lg border border-fuchsia-700/40 bg-primary-900 text-primary-100 p-3 shadow-2xl space-y-3 max-h-[80vh] overflow-y-auto">
      <div class="font-bold text-fuchsia-300">Dev quick-actions</div>

      <!-- Quick login per role -->
      <div class="space-y-1">
        <label class="block text-primary-400">Login as role</label>
        <div class="flex gap-1">
          <select v-model="role" class="flex-1 bg-primary-800 text-primary-100 placeholder:text-primary-500 border border-primary-700 rounded px-1 py-1">
      ...
      <div class="flex gap-1">
        <input v-model="userId" placeholder="userId" class="flex-1 bg-primary-800 text-primary-100 placeholder:text-primary-500 border border-primary-700 rounded px-1 py-1" />
      ...
      <!-- Trigger infer -->
      <div class="flex gap-1">
        <input v-model="imageId" placeholder="imageId" class="flex-1 bg-primary-800 text-primary-100 placeholder:text-primary-500 border border-primary-700 rounded px-1 py-1" />
        <button class="btn" @click="run(() => dev.infer(imageId))">infer</button>
      </div>

      <!-- Freeze clock -->
      <div class="space-y-1">
        <label class="block text-primary-400">Clock</label>
        <div class="flex gap-1">
          <input v-model="clockAt" type="datetime-local" class="flex-1 bg-primary-800 text-primary-100 placeholder:text-primary-500 border border-primary-700 rounded px-1 py-1" />
      ...
      <pre v-if="result" class="bg-primary-950/70 text-primary-200 rounded p-2 whitespace-pre-wrap break-all">{{ result }}</pre>
    </div>
  </div>
</template>
...
<style scoped>
.btn {
  @apply cursor-pointer rounded bg-accent-600 px-2 py-1 text-white transition-colors hover:bg-accent-500;
}
</style>
