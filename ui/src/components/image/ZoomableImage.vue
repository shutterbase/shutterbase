...
watch(
  ...
  () => {
    if (zoomed.value) {
      // navigating while zoomed keeps zoom and stage — the next image swaps
      // in-place with no fitted intermediate step. Unfreeze so it takes its
      // own stage-fitted size (aspect may differ); re-clamped on load.
...
// after the swapped-in image has its real fitted size, pull the kept offsets
// back into the new image's pan range
function onImgLoad() {
