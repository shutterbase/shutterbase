  ...
  <div class="mx-auto w-full max-w-7xl px-4 pt-8 pb-12 sm:px-6 lg:px-8">
    ...
    <template v-else>
      <div class="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
        ...
        >
          <FaceClusterGrid v-if="person.sample" :items="[person.sample]" :cols="1" />
          ...
          <li v-for="m in merges" :key="mergeKey(m)" class="py-3">
            ...
            <div v-if="expanded && expanded.key === mergeKey(m)" class="mt-3">
              <p class="label-mono-sm mb-2 text-primary-500 dark:text-primary-400">
                cluster {{ expanded.side }} — {{ expanded.total }} appearance{{ expanded.total === 1 ? "" : "s" }} across your projects
              </p>
...
import { AiMerge, AiMergeCandidate, AiPersonImage, AiRankedPerson } from "src/api/ai";
...
async function loadReview() {
  ...
  try {
    ...
    let offset = 0;
    let next = await api.ai.mergeNext(offset);
    while (next.candidate && skipped.has(pairKey(next.candidate)) && offset < 200) {
      offset++;
      next = await api.ai.mergeNext(offset);
    }
    remaining.value = next.remaining;
    if (next.candidate) {
      const [a, b] = await Promise.all([api.ai.personImagesGlobal(next.candidate.personA, 0, 4), api.ai.personImagesGlobal(next.candidate.personB, 0, 4)]);
...
async function toggleExpand(m: AiMerge, side: "A" | "B") {
  ...
  try {
    ...
    const page = await api.ai.personImagesGlobal(personRef, 0, 8, true);
...
async function decide(verdict: "same" | "different") {
  ...
  busy.value = true;
  try {
    await api.ai.mergeDecide(candidate.value.personA, candidate.value.personB, verdict);
    reviewed.value++;
    if (verdict === "same") {
...
async function unmerge(m: AiMerge) {
  ...
  try {
    ...
    expanded.value = null;
    await Promise.all([loadMerges(), loadReview(), loadPersons(true)]);
  } catch (error: any) {
    fail(error);
  } finally {
    busy.value = false;
...
async function skipPair() {
  if (!candidate.value) return;
  skippedStore.value = [...skippedStore.value, pairKey(candidate.value)].slice(-300);
  await loadReview();
}
