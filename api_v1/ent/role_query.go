...
func (rq *RoleQuery) QueryProjectAssignments() *ProjectAssignmentQuery {
	query := (&ProjectAssignmentClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withProjectAssignments: rq.withProjectAssignments.Clone(),
...
func (rq *RoleQuery) WithProjectAssignments(opts ...func(*ProjectAssignmentQuery)) *RoleQuery {
	query := (&ProjectAssignmentClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withProjectAssignments = query
	return rq
}
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := rq.withProjectAssignments; query != nil {
		if err := rq.loadProjectAssignments(ctx, query, nodes,
			func(n *Role) { n.Edges.ProjectAssignments = []*ProjectAssignment{} },
			func(n *Role, e *ProjectAssignment) {
...
func (rq *RoleQuery) loadProjectAssignments(ctx context.Context, query *ProjectAssignmentQuery, nodes []*Role, init func(*Role), assign func(*Role, *ProjectAssignment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Role)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.ProjectAssignment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.ProjectAssignmentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.project_assignment_role
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_assignment_role" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_assignment_role" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
