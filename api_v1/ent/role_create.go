...
func (rc *RoleCreate) AddProjectAssignmentIDs(ids ...uuid.UUID) *RoleCreate {
	rc.mutation.AddProjectAssignmentIDs(ids...)
	return rc
}

// AddProjectAssignments adds the "projectAssignments" edges to the ProjectAssignment entity.
func (rc *RoleCreate) AddProjectAssignments(p ...*ProjectAssignment) *RoleCreate {
	ids := make([]uuid.UUID, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return rc.AddProjectAssignmentIDs(ids...)
}
...
func (rc *RoleCreate) createSpec() (*Role, *sqlgraph.CreateSpec) {
	...
	if nodes := rc.mutation.ProjectAssignmentsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: true,
