...
func (uq *UserQuery) QueryProjectAssignments() *ProjectAssignmentQuery {
	query := (&ProjectAssignmentClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryCameras() *CameraQuery {
	query := (&CameraClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithProjectAssignments(opts ...func(*ProjectAssignmentQuery)) *UserQuery {
	query := (&ProjectAssignmentClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withProjectAssignments = query
	return uq
}
...
func (uq *UserQuery) WithCameras(opts ...func(*CameraQuery)) *UserQuery {
	query := (&CameraClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withProjectAssignments; query != nil {
		if err := uq.loadProjectAssignments(ctx, query, nodes,
	...
	if query := uq.withCameras; query != nil {
...
func (uq *UserQuery) loadProjectAssignments(ctx context.Context, query *ProjectAssignmentQuery, nodes []*User, init func(*User), assign func(*User, *ProjectAssignment)) error {
	...
	for _, n := range neighbors {
		fk := n.project_assignment_user
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_assignment_user" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_assignment_user" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadImages(ctx context.Context, query *ImageQuery, nodes []*User, init func(*User), assign func(*User, *Image)) error {
	...
	for _, n := range neighbors {
		fk := n.image_user
		if fk == nil {
			return fmt.Errorf(`foreign-key "image_user" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "image_user" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (uq *UserQuery) loadCameras(ctx context.Context, query *CameraQuery, nodes []*User, init func(*User), assign func(*User, *Camera)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		fk := n.camera_owner
		if fk == nil {
			return fmt.Errorf(`foreign-key "camera_owner" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (uq *UserQuery) loadCreatedBy(ctx context.Context, query *UserQuery, nodes []*User, init func(*User), assign func(*User, *User)) error {
	...
	nodeids := make(map[uuid.UUID][]*User)
	for i := range nodes {
		if nodes[i].user_created_by == nil {
			continue
		}
		fk := *nodes[i].user_created_by
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_created_by" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
