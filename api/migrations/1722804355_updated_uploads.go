...
func init() {
	m.Register(func(db dbx.Builder) error {
		...
		collection.ListRule = types.Pointer("@request.auth.role.key = \"admin\" || @request.auth.role.key = \"projectAdmin\" || @request.auth.id = user.id")
		...
		collection.CreateRule = types.Pointer("@request.auth.role.key = \"admin\" || @request.auth.id = user.id && user.projectAssignments.id ?= project.id")

		collection.UpdateRule = types.Pointer("@request.auth.role.key = \"admin\" || @request.auth.role.key = \"projectAdmin\" || @request.auth.id = user.id")

		collection.DeleteRule = types.Pointer("@request.auth.role.key = \"admin\" || @request.auth.role.key = \"projectAdmin\" || @request.auth.id = user.id")
