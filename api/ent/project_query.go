...
func (pq *ProjectQuery) QueryAssignments() *ProjectAssignmentQuery {
	query := (&ProjectAssignmentClient{config: pq.config}).Query()
...
func (pq *ProjectQuery) QueryImages() *ImageQuery {
...
func (pq *ProjectQuery) QueryTags() *ImageTagQuery {
	query := (&ImageTagClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProjectQuery) WithAssignments(opts ...func(*ProjectAssignmentQuery)) *ProjectQuery {
	query := (&ProjectAssignmentClient{config: pq.config}).Query()
...
func (pq *ProjectQuery) WithImages(opts ...func(*ImageQuery)) *ProjectQuery {
	query := (&ImageClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProjectQuery) WithTags(opts ...func(*ImageTagQuery)) *ProjectQuery {
	query := (&ImageTagClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := pq.withAssignments; query != nil {
		if err := pq.loadAssignments(ctx, query, nodes,
			func(n *Project) { n.Edges.Assignments = []*ProjectAssignment{} },
			func(n *Project, e *ProjectAssignment) { n.Edges.Assignments = append(n.Edges.Assignments, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := pq.withTags; query != nil {
		if err := pq.loadTags(ctx, query, nodes,
			func(n *Project) { n.Edges.Tags = []*ImageTag{} },
			func(n *Project, e *ImageTag) { n.Edges.Tags = append(n.Edges.Tags, e) }); err != nil {
			return nil, err
		}
	}
...
func (pq *ProjectQuery) loadAssignments(ctx context.Context, query *ProjectAssignmentQuery, nodes []*Project, init func(*Project), assign func(*Project, *ProjectAssignment)) error {
	...
	for _, n := range neighbors {
		fk := n.project_assignment_project
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_assignment_project" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_assignment_project" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (pq *ProjectQuery) loadImages(ctx context.Context, query *ImageQuery, nodes []*Project, init func(*Project), assign func(*Project, *Image)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		fk := n.image_project
		if fk == nil {
			return fmt.Errorf(`foreign-key "image_project" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (pq *ProjectQuery) loadTags(ctx context.Context, query *ImageTagQuery, nodes []*Project, init func(*Project), assign func(*Project, *ImageTag)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		fk := n.image_tag_project
		if fk == nil {
			return fmt.Errorf(`foreign-key "image_tag_project" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "image_tag_project" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
